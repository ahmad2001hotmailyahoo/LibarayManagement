...
enum  Month
{
	jan = 1 , feb , mar , april , may , jun , jul , aug , sep , ct , nov , ec
};
struct user
{
	char username[30] ; 
...
bool loadUser(user arr[] , int size ) 
{	
	ifstream fout ;
	fout.open("user.txt") ;
	for(int i = 0 ; i <= 100 ; i++)
	{
		...
		{
			fout.getline(arr[i].username , 27 ,','   );
			fout.getline(arr[i].Password , 30 , '\n' ); 		
		}
	}	
	return 0 ;
}

bool findUser(user arr[] , int size , user userToFind )
{
	bool s = false ;
	for(int i = 0 ; i < 30 ; i++)
	{
		if(!strcmp(arr[i].Password , userToFind.Password)) 
		{
			cout << "Valid " << endl ;
			s = true ; 
			break ; 	
...
bool issueBook(BookIssue arr[], int size, BookIssue toIssue)
{
	...
	for(int i = 0 ; i < 100 ; i++ )
	{
		if(!strcmp(arr[i].RegNo , toIssue.RegNo   ))
		{
			strcpy(arr[i].RegNo , a  )     ;
			strcpy(arr[i].bookCopyId , a ) ;
			arr[i].status = issues         ;
			string l =  __DATE__ ;
			strcpy(arr[i].IssueDate.D , __DATE__) ;
		}
	}
}
int returnBook(BookIssue arr[], int size, BookIssue toReturn)
{
	...
	for(int i = 0 ; i < 100 ; i++ )
	{
		if(!strcmp(arr[i].RegNo , toReturn.RegNo   ))
		{
			strcpy(arr[i].RegNo , a  )     ;
			strcpy(arr[i].bookCopyId , a ) ;
			arr[i].status = Returned       ;
			strcpy(arr[i].IssueDate.D , a )                      ;
			k = false ;
		}
	}
}
void studentCurrentBook(BookIssue arr[], int size, char regNo[])
{
	for(int i = 0 ; i < 100 ; i++)
	{
		...
		{
			if(arr[i].status == issues)
			{
				cout << arr[i].RegNo <<"      "  ;
				cout << arr[i].bookCopyId <<"     " ;
				cout << arr[i].IssueDate.D << endl ;	
			}
		}	
	}	
}
void checkBookStatus(BookIssue arr[], int issueSize, char copyId[])
{
	for(int i = 0 ; i < 100 ; i++)
	{
		if(arr[i].status == issues)
		{
			cout << arr[i].RegNo << "has this book" << endl ;
		}
		if(arr[i].status ==  Returned )
		{
			cout << arr[i].bookCopyId << "is availaible " << endl ;
		}
	}
}
void loadBookIssue(BookIssue arr[], int size)
{
	...
	char l [10] = {0} ;
	fstream fout ;
	fout .open("status.csv");
	for(int i = 0 ; i <= 100 ; i++)
	{
		fout.getline( arr[i].RegNo ,  20 , ','  ) ;
		fout.getline(arr[i].bookCopyId , 20 , ',');
		fout.get(f) ;
		if(f == 'i' )
			arr[i].status = issues 		;
		if(f == 'R' )
			arr[i].status = Returned	;
		fout.getline( l , 12 , ',')     ;
		fout.getline(arr[i].IssueDate.D , 21 , '\n' )                  ;
...
void saveBookIssue(BookIssue arr[] , int size)
{
	...
	for(int i = 0 ; i <= 100 ; i++)
	{
		if(!strcmp(arr[i].RegNo , a ) )
		{
				fout << arr[i].RegNo <<","       ;
				fout << arr[i].bookCopyId << "," ;
				fout << arr[i].status << ","     ;
				fout << arr[i].IssueDate.D << ","  ;
		}
	}
}
int main()
{
	...
	char f       ;
	BookIssue toIssue ;
	BookIssue copyIdtoReturn ;
	bool k =loadUser(User , size) ;
		cout << "Enter the User Name : "	<< endl ;
		cin.getline(Usertofind.username , 30 ,'\n'  ) ;
		cout << "Enter the password " << endl ;
		cin.getline(Usertofind.Password , 30 , '\n') ;
		bool o = findUser( User ,  size , Usertofind ) ;
		if( o == 1 ) 
		{
			cout << "(Categories ( A , E , L , D , P , S , L )" << endl ;
			cout << "(Books ( A , E , L , D , P , S , L )" << endl ;
			cout << "(BooksCopy ( A , E , L , D , P , S , L )" << endl ;
			cout << "(Student ( A , E , L , D , P , S , L )" << endl ;
			cout << "(Book Issue Manegement( A , E , L , D , P , S , L )" << endl ;
			cout << "(Layout  (9L)" << endl   ;
			cout << "(To Escit ( 0E)" << endl ;
		    BookIssue arr[100] = {0}      ;
			int size                      ;
			BookIssue toIssue            ; 
			char CN[20] ;
		...
		if( r == '4' && f == 'U')
			...
			if( r == '5' && f == 'I' )
			{
				...
				cin.getline(toIssue.RegNo , 20 , '\n') ;
				cout << "Enter the Book Copy Id : " << endl ;
				cin.getline(toIssue.bookCopyId , 20 , '\n')       ;
				bool p =issueBook( arr,  size,  toIssue) ;
			}
			if(r == '5' && f == 'R')
			{
				cout << "Enter the reg No : " << endl ;
				cin.getline(toIssue.bookCopyId, 18, '\n') ;
				int n = returnBook( arr,  size, toIssue ) ;
			}
			if( r == '5' && f == 'B')
			{
				char regNo[20] ;
				cout << "Enter the regiteration no." << endl ;
				cin.getline(regNo , 20 , '\n') ;
			 	studentCurrentBook( arr,  size,  regNo) ;
			}
			if(r == '5' && f == 'C') 
			{
			char copyId[20] ;
			cout << "Enter the copy Id" << endl ;
			cin.getline(copyId , 20 , '\n') ;	
			 checkBookStatus( arr,  size, copyId) ;
				
			}
			if( r == '5' && f == 'L' )
				loadBookIssue( arr, size) ;
			if(r == '5' && f == 'S' )
			   saveBookIssue( arr ,size);
